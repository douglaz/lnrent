...
- **Payment backend secret** — the **Fedimint client root secret** (the primary backend,
  ...
  include it. (phoenixd — a co-equal backend since ADR-0018, not a secondary one — keeps its OWN
  channel-state seed, generated by phoenixd and backed up separately. That separation is real
  and is the current behaviour: `lnrentd backup` does not cover phoenixd funds. Deriving that
  seed from the operator seed is lnrent-kr1, and even once it lands `phoenix.conf` still dies
  with the data dir, so the api password must be backed up too — lnrent-u43.)
...
refund pay; refunds MUST use it, never bare `pay`). The trait itself is the list of those methods;
docs/specs/refund-money-path-hardening.md §3 is the source of truth for their contracts.
...
- **phoenixd (co-equal alternative, standalone; ADR-0018):** self-custodial, HTTP API +
...
- **SUSPENDED -> RESUMING -> ACTIVE** — a late renewal settles within retention. Capture applies
  ...
  no request id at all, so its notice OMITS `request_id` entirely — the field is ABSENT from the
  JSON, not present as `null` — and arrives as an unsolicited async DM. Neither is a state transition — the resume driver keeps sole CAS ownership of
  `state='RESUMING'`.
...
- **Issuance ordering (the `bolt11` comes from the backend, so it can't be cached before the
  ...
  **backend-specific**: phoenixd is queried by `externalId` and the retry **recovers a live or
  ...
  `phoenixd_backend.rs:968`), so a retry after the orphan expired hands the buyer a payable
  ...
  because nobody holds its bolt11 (`lnv2_backend.rs:35` module doc). With a committed map row,
...
CREATE TABLE refund_attempt (        -- durable refund ledger (ADR-0009, §6.6; resolver cols lnrent-ug8)
  ...
  migration_unverified_at INTEGER,   -- ADR-0022 fence (built, lnrent-chgb): set by the legacy import on a non-terminal OR retryable FAILED attempt whose
                                     ...
                                     --   by the operator's explicit `lnrent migration clear-fence` (journaled); never by absence, never by a retry
  created_at INTEGER, updated_at INTEGER);
...
-- Every table whose comment cites ADR-0023 is DECIDED, NOT YET BUILT (the current daemon derives its view from alert history).
CREATE TABLE condition (             -- open money conditions a human may have to act on (ADR-0023; CONTEXT: Condition)
  ...
  closed_out_at INTEGER, closed_out_outcome TEXT,  -- the close-out marker, written only by `lnrentd condition closeout`
                                     --   (vocabulary and meaning: ADR-0024, not yet written; bead lnrent-zhy8); on the condition row, never the invoice (ADR-0023 §6)
...
-- What a close-out does to the books (booking, offsets) is ADR-0024 (not yet written; bead lnrent-zhy8): no schema here until it is decided.
...
`lnv2_invoice` alone carries a status, phoenixd's record is the status) and pay map (`phoenixd_pay`, `lnv2_pay`: refund/sweep key -> the backend's payment
...
> **The ladder below is the ORIGINAL plan and is currently SUSPENDED after M1a.** The operative
> sequence is a phased self-dogfood programme, recorded in the beads rather than restated here (it
> would drift): do-vps lifecycle on fedimint (`lnrent-9q9`), the same on phoenixd (`lnrent-vam`),
> multi-recipe serving so one daemon sells several services (`lnrent-ja2`), then a third service
> (`lnrent-5y9`). Hardened hosts and advanced service configurations follow and are deliberately
> unscoped. A stranger-facing launch — what `lnrent-ea1`, `lnrent-2em` and `lnrent-ehu` serve — comes
> after all of that.
>
> So M1b's Incus VM work is NOT next, and the multi-recipe capability the programme needs does not
> appear on this ladder at all. Read `lnrent-9q9` for the current order before planning from here.
...
- **M0 — Skeleton.** Repo, `lnrentd` skeleton, sqlite, recipe loader, and the
  ...
  `host` compute, WireGuard network, and `Fedimint` payment stubs (phoenixd was slated for M3
  here; ADR-0018 later promoted it to a co-equal backend and it shipped early — lnrent-xk3).
...
- **M3 — Further payment backends.** An optional **LND** backend (native hold invoices)
  for true provision-then-capture atomicity (§6.4). NOTE: **phoenixd is no longer an M3
  item** — ADR-0018 promoted it to a co-equal alternative to fedimint-lnv2 and it SHIPPED
  (lnrent-xk3), gated at runtime behind an explicit `accept_unsupported` opt-in until
  lnrent-tof's staging acceptance passes.
...
6. **Unified seed for payments (RESOLVED v0.23):** the Fedimint client root secret (the
   ...
   backs up identity + ecash funds (§4.6, ADR-0004/0012). phoenixd — co-equal since
   ADR-0018 — keeps a separate channel-state seed of its own, so one seed does NOT yet back
   up phoenixd funds (lnrent-kr1) and never backs up its api password (lnrent-u43).
