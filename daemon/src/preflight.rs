...
mod tests {
    ...
    /// A backend exposing ONLY the two reused readiness seams; every money method PANICS, so a
    /// passing test also proves preflight is read-only on the payment backend.
    struct SeamPayment {
        gateway: Seam,
        federation: Seam,
    }
    ...
    async fn malformed_token_fails_distinctly_without_probing() {
        ...
        for tok in [token("dop_v1\nsecret"), token("dop v1 secret"), token("dop_v1_\u{7f}")] {
            let v = preflight_report(&payment, &recipes, tok, &PanicProbe).await;
            assert_eq!(v["ok"], json!(false));
            let detail = checks(&v)[2]["detail"].as_str().unwrap();
