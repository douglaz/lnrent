...
- **The two files can disagree.** A v1 backup restored, a hand-copied data directory, a
  ...
  (`docs/go-live.md:287`, the "Index divergence" entry). The same condition exists for lnv2
  (`lookup_settlement_by_ref`, the known-id-with-no-row arm, `lnv2_backend.rs:1128`).
...
- **A replacement refreshes the invoice row, in that same transaction.** When the backend
  ...
  ADR-0023 extends this transaction: it also clears `provider_terminal_at` on the subject's
  `phoenixd_forgot_invoice` rows, so the polling obligation follows the successor (ADR-0023
  §2). Nothing for this ADR's own delivery; noted so the transaction stays the one place a
...
- One backend per Control node (CONTEXT.md, *Payment backend*), and **switching is not
  ...
  `config_conflict` (`config.rs:2456`). Rows of a foreign backend are retained defensively, not
...
- **Schema, static, all of it.** The store opens before any backend exists (`config.rs:992`
  `bootstrap_headless_with_store` opens it; `main.rs:450` constructs the backend afterwards), and
  ...
  is inherited only after the store is open (`config.rs:2462`, pinned by
  `inherited_phoenixd_still_requires_the_opt_in_on_every_start`, `config.rs:4205`), so a mode-keyed open would
...
- **Writes ride the caller's transaction.** `create_invoice(external_id, ...)` returns an
  ...
  - `persist: Box<dyn FnOnce(&rusqlite::Transaction) -> Result<()> + Send>` inserts (or, for
    ...
    **`Issued` owns the backend's per-`external_id` create guard as `lease`; `persist`
    ...
    `lnv2_backend.rs:1017` check→mint→insert): the guard is taken before the check and handed to
  ...
  - `after_commit: Box<dyn FnOnce() + Send>` runs **only after the transaction commits, and
    ...
    `OrderWrite::write`, `order_intake.rs:1056`; the lost soft-reminder CAS in
    `SoftReminderWrite::write`, `reconcile.rs:1515`), and starting the lnv2 receive watcher for
    ...
    (`store.rs:1071` `run`, ADR-0021 shutdown), and a cancelled caller would drop a closure it
  ...
  `phoenixd_backend.rs:1237`; lnv2 requires its caller to hold the equivalent): the guard is
  ...
  the fact. (PR #90 codex round 19.) **The lease covers the check-and-create only; the send is
  serialised separately.** `pay(key)` re-acquires the same `pay_start_lock`, and how long it
  holds it is backend-specific, exactly as today. **phoenixd** holds it across the provider
  call and through the terminal status write's commit (`pay_inner`,
  `phoenixd_backend.rs:1237`), because phoenixd has no idempotency-key layer of its own: two
  concurrent retries of one key that both read the row as `PREPARED` after the authorising
  commit would otherwise both POST `payinvoice`. **lnv2** holds it through the durable
  `PENDING` operation write (`start_send`, under the lock taken at `lnv2_backend.rs:703`)
  and releases it **before** awaiting finality (`await_send_final`, unlocked phase from
  `lnv2_backend.rs:760`): the committed `PENDING` row is the single-sender claim, and
  holding the mutex through a wait that can run to the production timeout would serialise
  every unrelated refund and sweep behind it. One mutex, taken twice in sequence: once for
  prepare, held until the authorising transaction commits (the lease); once for pay, held
  until that backend's claim is durable. Neither is dropped mid-send. (PR #90 codex round 99.) **No backend-owned path may create a correlation row** outside a
...
- **Status transitions on existing rows are the backend's own writes.** A receive reaching
  ...
  `PAID_UNRECOVERED` when the terminal arrives (`lnv2_backend.rs:1386` `spawn_receive_task`;
  `idx_mark_paid` / `idx_mark_canceled` / `idx_mark_paid_unrecovered` at `lnv2_backend.rs:1704`,
  `lnv2_backend.rs:1725` and `lnv2_backend.rs:1736`), and a pay reaching
...
wallet neither (`backup.rs:227`), unchanged.
...
- **Backups written under this design are manifest format v3.** Today the writer stamps v2
  iff `phoenixd_index.db` was captured, else v1 (`backup.rs:266`); with the side file gone, an
  ...
  offline path dispatched before any daemon boot (`main.rs:171` `Command::Backup`; `backup.rs:614`
  ...
  present, else v1 (`backup.rs:266`); an lnv2-only operator's pre-migration backup is v1, and v1 is
  ...
  v1/v2 distinction still gates restore: `backup.rs:444` accepts format-v1 backups, which carry
  ...
  'phoenixd-%'`, the prefix lives on the store's `id` column via `invoice_id_for`,
  `phoenixd_backend.rs:991`, while `backend_invoice_id` holds the bare payment hash,
  `phoenixd_backend.rs:1992`; or any
  ...
  **refuses the whole import** if any is missing, naming the first uncorrelated row and the
  ...
  - `invoice` rows with that backend's id prefix: every one must have a receive-map row,
    ...
    **current** receive for that key, i.e. its final state is `Claimed` (paid), or
    ...
    import repairs the books; the `lnv2_paid_unrecovered` condition for it is **not** this
    ADR's to write, since the `condition` table and its registry ship with ADR-0023, whose
    delivery backfills every `PAID_UNRECOVERED` row after this import has run), or it is still pending and
    ...
    (`lnv2_backend.rs:1021` and `lnv2_backend.rs:1467`), so "A's operation exists" is true of every stale map and proves
    ...
    asked, or answers for neither or both, the import refuses naming the row. **An answer
    for the map's row alone is not enough either** when the books hold a different invoice:
    phoenixd answering only for A, with B absent, is exactly the forgotten-B shape above, and
    absence of B proves nothing about whether B was paid before it vanished or will
    reappear paid. Repairing the books back to A there would leave B's settlement matching
    nothing. So with the books naming B and the map naming A, the import proceeds only when
    the backend positively establishes A **and** positively establishes that B never existed
    for it or is terminal unpaid, **from a source that survives whatever was rolled back**.
    Neither backend has one today: phoenixd forgets, and lnv2's receive state is read from
    the client's own persisted notifier (`await_receive_final`,
    `subscribe_receive_operation_state_updates`, `lnv2_backend.rs:2132`), which a rolled-back
    `client.db` can have lost along with the map (`lnv2_backend.rs:1282` records the same
    hazard for sends). So this shape **refuses for both backends** and the operator decides
    which file is current; the lnv2 half may relax only if
    `lnrent-lnv2-restore-fresh-hash-proof-l5kk` proves a federation- or gateway-side answer
    for a receive's terminal state. (PR #90 codex rounds 23, 57, 112 and 113.) **For phoenixd the row compared is the effective one, not any agreeing
    one**: `phoenixd_invoice` has no status and its upsert conflicts on `invoice_id`
    (`idx_upsert`, `ON CONFLICT(invoice_id)` at `phoenixd_backend.rs:2427`), so a replacement
    under the same `external_id` leaves the old row in place and `idx_get_invoice` reads the
    newest `rowid` as effective (`ORDER BY rowid DESC`, `phoenixd_backend.rs:2381`). An agreement check that
    ...
    `payment_hash` (`lnv2_backend.rs:1577`), so "a hash the map has never seen" would reject the
  ...
  - **every `SENT` `refund_attempt` / `sweep_attempt` row, keyed by its idempotency key**,
    ...
    (`refund.rs:416`, `sweep.rs:456`) and the SQL keeps an id only if one was recorded earlier, so a
    ...
    resolution_gen)` (`refund.rs:215`) yields the bare `refund:<external_id>` at generation 0
    ...
    codex round 69.) **And a `SENT` attempt's map row must be in its backend's success
    state** (`phoenixd_pay.status = 'SUCCEEDED'`, `lnv2_pay.status = 'SUCCEEDED'`), not
    merely present with matching fields: a stale file can hold an older `FAILED` row for the
    same key and bolt11 (a preflight failure followed by a successful same-key retry), and
    since the `[8A]` hash-owner lookup (`pay_other_key_for_hash`) excludes `FAILED` rows by
    design, importing that pair would leave the paid hash unowned and let a later key
    targeting the same bolt11 adopt the already-paid wallet record, crediting one payment
    twice. A `SENT`/`FAILED` (or `SENT`/`PENDING`) pair is a disagreement and refuses like
    any other. (PR #90 codex round 89.) A mismatched
  ...
  - every legacy `phoenixd_unbookable_settlement` timer must resolve to a subject: the timer
    stores only the phoenixd `invoice_id` (`phoenixd_backend.rs:322`), while ADR-0023 keys the
  ...
  - non-terminal **and retryable `FAILED`** attempts **without** a `backend_payment_id` whose
    ...
    liability A (`pay_get` reads only operation id and status, `lnv2_backend.rs:1773`, and an
    existing PREPARED operation is adopted without comparing bolt11, `lnv2_backend.rs:741`).
  ...
  - `FAILED` attempts **without** a `backend_payment_id` and **without** a pay-map row take
    ...
    (`ipc.rs:1164`) resets such a row to `PENDING`, and `plan_payment` may then re-resolve it to a
  ...
  - non-terminal attempts **without** a `backend_payment_id` and **without** a pay-map row:
    **not validated, on purpose.**
    ...
    successful return and `attempts` is bumped only after the send returns, on either path
    (`refund.rs:916`, `refund.rs:1050`), so a PENDING
    ...
    file lost. The import cannot tell them apart and must not pretend to. **That includes a
    row whose LNURL is still unresolved** (`resolved_bolt11` NULL, `dest` not a bolt11): the
    books say the send never started (`plan_payment` returns before any pay key or backend
    call, `refund.rs:378`, `refund.rs:407`), but the books can be a restore rolled back to
    before the resolution while the wallet paid afterwards (the rollback scenario uxbd
    records), and snapshot fields prove only what was known at backup time. Imported unfenced
    it would resolve a new generation and pay again. It stays fenced; for lnv2, whose
    clearance derives an operation from a concrete bolt11 (`lnv2_backend.rs:1270`), gjwy
    must decide how a row with none is cleared, and until it does such a row stays parked,
    which is the honest state. (PR #90 codex rounds 109 and 110.) **Until the
    ...
    posture), and the stamp is cleared in only two ways: the audit finds the send in the
    wallet's own outgoing records and **adopts** it (the attempt takes the record's payment
    id and its paid/failed outcome; nothing is re-sent), or the operator clears it
    explicitly (below). The stamp is a **column**, `migration_unverified_at` on
    ...
    by the audit's adoption or the operator's command; no driver path may reset it. **Absence never clears the fence for phoenixd.** Three reasons, each
    sufficient. uxbd records that phoenixd's outgoing list is filtered on `completedAt` and an
    in-flight payment has none, so a legacy POST still in flight is invisible to any list, and
    no provider-side maximum in-flight lifetime is known (`PAY_TIMEOUT`,
    `phoenixd_backend.rs:205`, is lnrent's HTTP client bound, called ambiguous in its own
    comment). Any time window would compare phoenixd's `completedAt` against lnrent's clock
    on a host with an unrelated wall clock (`phoenixd_backend.rs:653`), and both natural
    lower bounds fail (the manifest is stamped after the snapshot, `backup.rs:228`; an
    attempt's `created_at` can read later than the send's `completedAt` on the other clock).
    And even the wallet's whole retained history is not proof of continuity: phoenixd's own
    database can be wiped or restored while its funds survive by seed (the measured
    seed-restore behaviour), so a legacy send can have completed and no longer appear
    anywhere; "queried everything currently retained" is still absence-based authorisation
    of a replacement outlay. So the audit's only power is **adoption on a positive match**;
    an attempt it cannot match stays parked, `RefundStuck`/`SweepStuck` keep alerting, and
    the way out is the operator: `lnrent migration clear-fence <attempt id> --note "<what
    you checked>"`, same authority as `lnrent sweep --yes`, journaled to `event_log`, after
    which the driver re-prepares the attempt exactly as a first attempt. The operator is the
    only party who can weigh phoenixd's records, the wallet balance and the buyer's word;
    the daemon must not guess on their behalf. (PR #90 codex rounds 105, 110, 114, 120 and
    121.)
    **The clearing audit is per backend, and only
    ...
    *proof* bead and clears nothing itself; the lnv2 clearance is
    ...
    restore can roll back past an operation that happened (`lnv2_backend.rs:1282` says so on the
