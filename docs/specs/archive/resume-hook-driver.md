...
> **ARCHIVED — delivered.** Bead lnrent-18v. Shipped as `daemon/src/resume.rs` plus capture's
> RESUMING retention. SPEC.md §6.3 carries the standing state-machine contract, including the
> per-renewal refunds and first-baseline timer restore on permanent hook failure.
