...
live, code-cited definition. Moving it would break those citations to buy nothing.

**Original status (2026-07-03):** draft for codex-review-loop → beads. Written from a full
read-only review of the M1a tree by six focused audits (money-path state machine, refund/payment
backend, security/secrets, Nostr/wire transport, rate-limiting/abuse, wallet-ops/federation) plus
overengineering/prod-gaps. Every finding was independently verified against the code; each carries
a `file:line` anchor.
